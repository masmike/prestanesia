...
class BankBNI extends PaymentModule
{
	...
	public function __construct()
	{
		...
		if (isset($config['BANK_BNI_OWNER']))
			$this->owner = $config['BANK_BNI_OWNER'];
		if (isset($config['BANK_BNI_DETAILS']))
			$this->details = $config['BANK_BNI_DETAILS'];
		if (isset($config['BANK_BNI_ADDRESS']))
			$this->address = $config['BANK_BNI_ADDRESS'];

		parent::__construct();

		$this->displayName = $this->l('Bank BNI');
		$this->description = $this->l('Accept payments by bank BNI.');
		...
		$this->extra_mail_vars = array(
										'{bankwire_owner}' => Configuration::get('BANK_BNI_OWNER'),
										'{bankwire_details}' => nl2br(Configuration::get('BANK_BNI_DETAILS')),
										'{bankwire_address}' => nl2br(Configuration::get('BANK_BNI_ADDRESS'))
										);
	}
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('btnSubmit'))
		{
			Configuration::updateValue('BANK_BNI_DETAILS', Tools::getValue('details'));
			Configuration::updateValue('BANK_BNI_OWNER', Tools::getValue('owner'));
			Configuration::updateValue('BANK_BNI_ADDRESS', Tools::getValue('address'));
		}
	...
	{
		$this->_html .= '<img src="../modules/bankbni/bankwire.jpg" style="float:left; margin-right:15px;"><b>'.$this->l('This module allows you to accept payments by bank wire.').'</b><br /><br />
