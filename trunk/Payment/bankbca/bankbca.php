...
class BankBCA extends PaymentModule
{
	...
	public function __construct()
	{
		...
		if (isset($config['BANK_BCA_OWNER']))
			$this->owner = $config['BANK_BCA_OWNER'];
		if (isset($config['BANK_BCA_DETAILS']))
			$this->details = $config['BANK_BCA_DETAILS'];
		if (isset($config['BANK_BCA_ADDRESS']))
			$this->address = $config['BANK_BCA_ADDRESS'];

		parent::__construct();

		$this->displayName = $this->l('Bank BCA');
		$this->description = $this->l('Accept payments by bank BCA.');
		...
		$this->extra_mail_vars = array(
										'{bankwire_owner}' => Configuration::get('BANK_BCA_OWNER'),
										'{bankwire_details}' => nl2br(Configuration::get('BANK_BCA_DETAILS')),
										'{bankwire_address}' => nl2br(Configuration::get('BANK_BCA_ADDRESS'))
										);
	}
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('btnSubmit'))
		{
			Configuration::updateValue('BANK_BCA_DETAILS', Tools::getValue('details'));
			Configuration::updateValue('BANK_BCA_OWNER', Tools::getValue('owner'));
			Configuration::updateValue('BANK_BCA_ADDRESS', Tools::getValue('address'));
		}
	...
	{
		$this->_html .= '<img src="../modules/bankbca/bankwire.jpg" style="float:left; margin-right:15px;"><b>'.$this->l('This module allows you to accept payments by bank wire.').'</b><br /><br />
		...
		'<form action="'.Tools::htmlentitiesUTF8($_SERVER['REQUEST_URI']).'" method="post">
			...
			<legend><img src="../img/admin/contact.gif" />'.$this->l('Contact details').'</legend>
				<table border="0" width="500" cellpadding="0" cellspacing="0" id="form">
					<tr><td colspan="2">'.$this->l('Please specify the bank BCA account details for customers').'.<br /><br /></td></tr>
