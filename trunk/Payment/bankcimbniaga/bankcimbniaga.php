...
class BankCIMBNIAGA extends PaymentModule
{
	private $_html = '';
	...
	public function __construct()
	{
		...
		$config = Configuration::getMultiple(array('BANK_CIMBNIAGA_DETAILS', 'BANK_CIMBNIAGA_OWNER', 'BANK_CIMBNIAGA_ADDRESS'));
		if (isset($config['BANK_CIMBNIAGA_OWNER']))
			$this->owner = $config['BANK_CIMBNIAGA_OWNER'];
		if (isset($config['BANK_CIMBNIAGA_DETAILS']))
			$this->details = $config['BANK_CIMBNIAGA_DETAILS'];
		if (isset($config['BANK_CIMBNIAGA_ADDRESS']))
			$this->address = $config['BANK_CIMBNIAGA_ADDRESS'];
		...
		$this->extra_mail_vars = array(
										'{bankwire_owner}' => Configuration::get('BANK_CIMBNIAGA_OWNER'),
										'{bankwire_details}' => nl2br(Configuration::get('BANK_CIMBNIAGA_DETAILS')),
										'{bankwire_address}' => nl2br(Configuration::get('BANK_CIMBNIAGA_ADDRESS'))
										);
	}
	...
	{
		if (!Configuration::deleteByName('BANK_CIMBNIAGA_DETAILS')
				|| !Configuration::deleteByName('BANK_CIMBNIAGA_OWNER')
				|| !Configuration::deleteByName('BANK_CIMBNIAGA_ADDRESS')
				|| !parent::uninstall())
			return false;
		return true;
	}
	...
	private function _postProcess()
	{
		if (Tools::isSubmit('btnSubmit'))
		{
			Configuration::updateValue('BANK_CIMBNIAGA_DETAILS', Tools::getValue('details'));
			Configuration::updateValue('BANK_CIMBNIAGA_OWNER', Tools::getValue('owner'));
			Configuration::updateValue('BANK_CIMBNIAGA_ADDRESS', Tools::getValue('address'));
		}
	...
	{
		$this->_html .= '<img src="../modules/bankcimbniaga/bankwire.jpg" style="float:left; margin-right:15px;"><b>'.$this->l('This module allows you to accept payments by bank wire.').'</b><br /><br />
